...
const EmployeeRegister = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrors({ general: getExactErrorMessage(err) });
    } finally {
      setLoading(false);
    }
  };
      ...
      <main className="relative z-10 max-w-7xl w-full mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center my-auto py-6 sm:py-8">
        ...
        <div className="lg:col-span-6 w-full">
          <div className="bg-white rounded-3xl sm:rounded-[32px] p-6 sm:p-8 md:p-10 shadow-2xl shadow-indigo-950/30 text-slate-900 max-w-md w-full mx-auto lg:ml-auto relative">
            ...
            {errors.general && (
              <div className="p-3 mb-4 rounded-2xl bg-rose-50 border border-rose-200 text-rose-600 text-xs font-bold text-center leading-relaxed">
                {errors.general}
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-slate-100/80 hover:bg-slate-100 focus:bg-white text-slate-900 placeholder:text-slate-400 border border-transparent focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 rounded-2xl pl-4 pr-11 py-2.5 text-sm font-medium transition-all outline-none"
                  />
                  <button
                    type="button"
              ...
              <div className="pt-2">
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-indigo-600 hover:bg-indigo-700 active:scale-[0.99] text-white font-extrabold py-3 sm:py-3.5 px-6 rounded-2xl shadow-lg shadow-indigo-500/30 transition-all duration-200 cursor-pointer disabled:opacity-70 text-sm"
                >
